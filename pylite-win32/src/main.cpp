...
void ShowCompletion(const std::vector<std::wstring>&v,int prefixBytes=0){
  if(v.empty()){Sci(SCI_AUTOCCANCEL);return;}std::string list;for(size_t i=0;i<v.size()&&i<2000;i++){if(!list.empty())list+=' ';list+=U8(v[i]);}Sci(SCI_AUTOCSHOW,prefixBytes,(sptr_t)list.c_str());
}
...
LRESULT CALLBACK Proc(HWND h,UINT m,WPARAM w,LPARAM l){
  ...
  case WM_COMPLETIONS:{auto*v=(std::vector<std::wstring>*)l;if((unsigned)w==gCompletionRequest){if(v->empty())gCompletionKey.clear();std::wstring key;int prefixBytes=0;if(CurrentDynamicCompletion(key,prefixBytes)&&key==gCompletionKey){ShowCompletion(*v,prefixBytes);SetStatus(v->empty()?L"模块补全不可用，可重试":L"模块补全已加载");}}delete v;return 0;}
  case WM_IMPORT_COMPLETIONS:{auto*v=(std::vector<std::wstring>*)l;if((unsigned)w==gImportCompletionRequest){if(v->empty())gImportCompletionKey.clear();std::wstring key;int prefixBytes=0;if(CurrentImportCompletion(key,prefixBytes)&&key==gImportCompletionKey){ShowCompletion(*v,prefixBytes);SetStatus(v->empty()?L"可导入模块加载失败，可重试":L"可导入模块已加载");}}delete v;return 0;}
