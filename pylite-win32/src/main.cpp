...
LRESULT CALLBACK Proc(HWND h,UINT m,WPARAM w,LPARAM l){
  ...
  case WM_NOTIFY:{
    ...
    if(n->idFrom==ID_EDIT){auto*sc=(SCNotification*)l;
      ...
      else if(sc->nmhdr.code==SCN_CHARADDED){HandleAutoPair(sc->ch);if(sc->ch=='.')DynamicComplete();else if(sc->ch=='_'||(sc->ch>0&&sc->ch<128&&isalnum((unsigned char)sc->ch))){if(!DynamicComplete())CompleteStatic();}}
