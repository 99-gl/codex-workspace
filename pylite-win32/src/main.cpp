...
std::wstring gFile,gRoot,gProblemDir,gInterpreter,gEncoding=L"UTF-8",gStatusText=L"就绪",gPositionText=L"Ln 1, Col 1    UTF-8    Python"; bool gDirty=false,gOutputCollapsed=false,gRunning=false,gRunningTests=false,gShowInput=false,gFormatting=false; int gLeft=230,gRight=480,gSavedOutput=180; int gDrag=0,gHotMenu=-1,gLastExit=-1; POINT gDragStart{}; int gDragValue{}; UINT gDpi=96;
...
LRESULT CALLBACK Proc(HWND h,UINT m,WPARAM w,LPARAM l){
  ...
  case WM_NOTIFY:{
    ...
    if(n->idFrom==ID_EDIT){auto*sc=(SCNotification*)l;
      ...
      else if(sc->nmhdr.code==SCN_CHARADDED){if(sc->ch=='.')DynamicComplete();else if(sc->ch=='_'||(sc->ch>0&&sc->ch<128&&isalnum((unsigned char)sc->ch)))CompleteStatic();}
