...
std::wstring gFile,gRoot,gInterpreter,gEncoding=L"UTF-8",gStatusText=L"就绪",gPositionText=L"Ln 1, Col 1    UTF-8    Python"; bool gDirty=false,gOutputCollapsed=false,gRunning=false,gFormatting=false,gHighlightPending=false; int gLeft=230,gRight=260,gSavedOutput=180; int gDrag=0,gHotMenu=-1,gLastExit=-1; POINT gDragStart{}; int gDragValue{}; UINT gDpi=96; DWORD gLastEditTick=0;
...
void SetEditorText(const std::wstring&s){gFormatting=true;SetWindowTextW(gEdit,s.c_str());ApplyParagraphSpacing(gEdit);SendMessageW(gEdit,EM_SETMODIFY,FALSE,0);gDirty=false;gHighlightPending=true;gLastEditTick=GetTickCount();gFormatting=false;UpdateTitle();if(gGutter)InvalidateRect(gGutter,nullptr,FALSE);}
...
LRESULT CALLBACK Proc(HWND h,UINT m,WPARAM w,LPARAM l){
  ...
  case WM_CREATE:{
    ...
    SendMessage(gEdit,EM_SETEVENTMASK,0,ENM_CHANGE|ENM_SELCHANGE);SetControlFonts();ApplyParagraphSpacing(gEdit);ApplyParagraphSpacing(gOutput);SetWindowSubclass(gEdit,EditProc,1,0);
  ...
  case WM_TIMER:if(gHighlightPending&&GetTickCount()-gLastEditTick>=300){gHighlightPending=false;Highlight();}return 0;
  case WM_SETCURSOR:{
  ...
  case WM_NOTIFY:{
    ...
    if(n->idFrom==ID_EDIT&&n->code==EN_SELCHANGE){CHARRANGE c{};SendMessage(gEdit,EM_EXGETSEL,0,(LPARAM)&c);int line=(int)SendMessage(gEdit,EM_LINEFROMCHAR,c.cpMin,0),base=(int)SendMessage(gEdit,EM_LINEINDEX,line,0);gPositionText=L"Ln "+std::to_wstring(line+1)+L", Col "+std::to_wstring(c.cpMin-base+1)+L"    "+gEncoding+L"    Python";InvalidateRect(gGutter,nullptr,FALSE);auto u=MeasureLayout();InvalidateRect(h,&u.status,FALSE);}
    return 0;
  }
  case WM_COMMAND:{
    int id=LOWORD(w);if(id==ID_EDIT&&HIWORD(w)==EN_CHANGE&&!gFormatting){gDirty=SendMessage(gEdit,EM_GETMODIFY,0,0);gHighlightPending=true;gLastEditTick=GetTickCount();UpdateTitle();return 0;}
